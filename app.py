...
def index():
    ...
    groups = (
        ...
        .join(User_Group)
        .filter(User_Group.user_id == current_user.id)
        .all()
    )
    ...
    qrytext = text(
        ...
        FROM groups JOIN expenses ON groups.id = expenses.group_id WHERE groups.id IN "
        + group_ids
        + "\
    ...
    debt_amount = (
        Debt.query.with_entities(func.sum(Debt.amount).label("debt"))
        .filter(Debt.user_id == current_user.id, Debt.settled == False)
        .first()
        .debt
    )
    credit_amount = (
        db.session.query(Debt)
        .with_entities(func.sum(Debt.amount).label("credit"))
        .join(Expense)
        .filter(Expense.payer_id == current_user.id, Debt.settled == False)
        .first()
        .credit
    )
...
def group(group_id):
    ...
    members = (
        ...
        .join(User_Group)
        .filter(User_Group.group_id == group_id)
        .all()
    )
    ...
    return render_template(
        ...
        debts=debts,
        members=members,
        expenses_categorised=expenses_categorised,
...
def add_member():
    ...
    # Ensure user to be added is not already a part of this group
        ...
        User_Group.query.filter_by(user_id=user.id, group_id=group_id).first()
        is not None
    ):
...
def add_expense():
    ...
    # If expense is to be split equally between a subset of members
        ...
        for id in debtor_ids:
            new_debts.append(
                Debt(group_id, expense.id, id, (amount / len(debtor_ids)), False, False)
            )
